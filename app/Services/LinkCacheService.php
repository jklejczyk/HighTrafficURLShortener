...
class LinkCacheService
{
    ...
    public function get(string $code): ?string
    {
        ...
        if ($cached === self::NOT_FOUND_SENTINEL) return null;
    ...
    {
        if ($link->expires_at?->isPast()) return;
    ...
    private function calculateTtlInSeconds(\Carbon\CarbonInterface|null $expiresAt): int
    {
        if (!$expiresAt) return self::POSITIVE_TTL_HOURS * 3600;
